...
char TicTacToeGame::getWinner()
{
    ...
    for(int i = 0; i < 3; i++)
    {
        ...
        if(currentState.mBoard[i][0] == GameState::X && currentState.mBoard[i][1] == GameState::X
           && currentState.mBoard[i][2] == GameState::X)
        {
            return 'X';
        }
        //check horizontals for O win
        if(currentState.mBoard[i][0] == GameState::O && currentState.mBoard[i][1] == GameState::O
           && currentState.mBoard[i][2] == GameState::O)
        {
            return 'O';
        }
        //check verticals for X win
        if(currentState.mBoard[0][i] == GameState::X && currentState.mBoard[1][i] == GameState::X
           && currentState.mBoard[2][i] == GameState::X)
        {
            return 'X';
        }
        //check verticals for O win
        if(currentState.mBoard[0][i] == GameState::O && currentState.mBoard[1][i] == GameState::O
    ...
    if (currentState.mBoard[0][0] == GameState::X && currentState.mBoard[1][1] == GameState::X
        && currentState.mBoard[2][2] == GameState::X)
    {
        return 'X';
    }
    else if (currentState.mBoard[0][2] == GameState::X && currentState.mBoard[1][1] == GameState::X
        && currentState.mBoard[2][0] == GameState::X)
    {
        return 'X';
    }
    //check diagonals for O win
    if (currentState.mBoard[0][0] == GameState::O && currentState.mBoard[1][1] == GameState::O
        && currentState.mBoard[2][2] == GameState::O)
    {
        return 'O';
    }
    else if (currentState.mBoard[0][2] == GameState::O && currentState.mBoard[1][1] == GameState::O
